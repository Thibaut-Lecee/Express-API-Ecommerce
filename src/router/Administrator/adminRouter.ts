...
import {validatePurchase} from "../../Middlewares/handleData";
...
adminRouter.route("/modifyArticle/:id").put(modifyArticle);
...
adminRouter.route("/modifyPurchase/:id").put(validatePurchase, modifyPurchase);
...
adminRouter.route("/signUp").post(createUser);
adminRouter.route("/signIn").post(signIn);
adminRouter.route("/modifyUser").put(modifyUser);
